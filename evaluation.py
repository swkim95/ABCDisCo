...
def ABCDfunc_bg(bgout1,bgout2,y1list,y2list):
    ...
    Aa=bg1[(bg1>y1upper) & (bg2>y2upper)] # the SR
    Bb=bg1[(bg1>y1upper) & (bg2<y2upper)] # VR
    Cc=bg1[(bg1<y1upper) & (bg2>y2upper)] # VR
    Dd=bg1[(bg1<y1upper) & (bg2<y2upper)] # the CR

    return len(Aa),len(Bb),len(Cc),len(Dd)

def ABCDfunc_sig(sigout1,sigout2,y1,y2):
    
    Aa=sigout1[(sigout1>y1) & (sigout2>y2)] # the SR
    Bb=sigout1[(sigout1>y1) & (sigout2<y2)] # VR
    Cc=sigout1[(sigout1<y1) & (sigout2>y2)] # VR
    Dd=sigout1[(sigout1<y1) & (sigout2<y2)] # the CR

    return len(Aa),len(Bb),len(Cc),len(Dd)

# create a list of y1,y2 cut values based on signal efficiency eS
def create_y1y2list(sigout1,sigout2,eS,Nval):
    y1y2list=[]
    for eS1 in np.linspace(eS,1,Nval):
        eS2=eS/eS1
        y1cut=np.percentile(sigout1,100-eS1*100)
        y2cut=np.percentile(sigout2[sigout1>y1cut],100-eS2*100)
    #    print(len(output1[(output1>y1cut) & (output2>y2cut)])/len(output1))
        y1y2list.append([y1cut,y2cut])
    for eS2 in np.linspace(eS,1,Nval):
        eS1=eS/eS2
        y2cut=np.percentile(sigout2,100-eS2*100)
        y1cut=np.percentile(sigout1[sigout2>y2cut],100-eS1*100)
    #    print(len(output1[(output1>y1cut) & (output2>y2cut)])/len(output1))
        y1y2list.append([y1cut,y2cut])
...
def calculate_eB_eABCD(sigout1,sigout2,bgout1,bgout2,y1y2list,y1lower,y2lower):
    eBlist=[]
    eRlist=[]
    eRsiglist=[]
...
#        y1lower=0
#        y2lower=0
        Abg,Bbg,Cbg,Dbg=ABCDfunc_bg(bgout1,bgout2,[y1lower,y1cut],[y2lower,y2cut])
        Asig,Bsig,Csig,Dsig=ABCDfunc_sig(sigout1,sigout2,y1cut,y2cut)
    #    print(Asig/(Asig+Bsig+Csig+Dsig))
#        eStry=Asig/(Asig+Bsig+Csig+Dsig)
        eBlist.append([y1lower,y1cut,y2lower,y2cut,Abg/len(bgout1),np.sqrt(Abg)/len(bgout1)])
...
def ABCD_metrics(output1,output2,labels):
    ...
    for eStry in [0.1,0.3,0.6]:
        ...
        y1y2list=create_y1y2list(sigout1,sigout2,eStry,100)
        eBlist_temp,eRlist_temp,eRsiglist_temp=calculate_eB_eABCD(sigout1,sigout2,bgout1,bgout2,y1y2list,-1,-1)
        eBlist+=eBlist_temp
        eRlist+=eRlist_temp
