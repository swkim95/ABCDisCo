...
def calculate_loss(net,data,target,weight,mass,alpha=0.,decorr_mode='none'):
    ...
    loss1 = F.binary_cross_entropy(output,target.float(),weight)
...
#        loss = F.cross_entropy(output, target)
        ...
        loss1,loss2,_=calculate_loss(net,data,target,weight,mass,alpha,decorr_mode)
        loss=loss1+loss2
        ...
        loss1_avg = loss1_avg * 0.2 + float(loss1) * 0.8
...
#        target = int(target)
        ...
        with torch.no_grad():
            ...
            loss1,loss2,output=calculate_loss(net,data,target,weight,mass,alpha,decorr_mode)
...
#    state['val_accuracy'] = correct / Nvalcount
    ...
    return outputall,labels,weights,binnums,masses,loss1_avg,loss2_avg
...
#    val_acc_list=[]
    ...
    for epoch in range(Nepochs):
        ...
        train_loss,train_loss2=train(net,optimizer,train_loader,decorr_mode,alpha=alpha)

        output,labels,weights,_,masses,val_loss1,val_loss2\
...
#        print(np.mean(qcdout))
        ...
        output1=output
#        output2=-np.abs(masses-175)
#        output2=1-output2/min(output2)
        output2=masses
    
        eBlist,eRlist,eRsiglist=ABCD_metrics(output1,output2,labels)
        np.save('eBlist_'+label+'_'+str(alpha)+"_"+str(epoch)+'_mD.npy',eBlist)
        np.save('eRlist_'+label+'_'+str(alpha)+"_"+str(epoch)+'_mD.npy',eRlist)
        np.save('eRsiglist_'+label+'_'+str(alpha)+"_"+str(epoch)+'_mD.npy',eRsiglist)
        
        t3 = time.time()
        print('Calculating metrics time: {} seconds'.format(t3 - t2))
        

        print("Train loss, val loss, val accuracy, val R50, val R30: ",[train_loss,train_loss2],[val_loss1,val_loss2],val_acc,R50,R30)
        ...
        state['val_loss',alpha,epoch]=val_loss1
...
#        if(epoch>10 and np.mean(val_acc_list[-10:])<0.52):
#            break
        val_loss=val_loss1+val_loss2
        if val_loss < best_loss and val_loss2>=0:
            best_loss=val_loss
            best_epoch=epoch
            torch.save(net.state_dict(),  "decorrelation_model_"+label+'_'+str(alpha)+"_"+str(epoch)+"_bestvalloss.dict")
        #    log.write('%s\n' % json.dumps(state))
        #    log.flush()
        #    print(state)
            print("Best loss, alpha, epoch: %f" % best_loss,val_loss1,val_loss2, alpha, best_epoch)
