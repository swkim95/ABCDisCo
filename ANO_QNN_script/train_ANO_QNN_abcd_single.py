...
from QNN_networks import ANO_VQC_Model
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # ANO VQC configuration (for QNN head)
    parser.add_argument("--qnn-n-qubits", type=int, default=11, help="Number of qubits / input angles consumed")
    parser.add_argument("--qnn-n-outputs", type=int, default=2, help="Number of output logits (first n_outputs measurements)")
    parser.add_argument("--qnn-n-local", type=int, default=1, help="Locality (window size) per learned observable")
    parser.add_argument("--qnn-vqc-depth", type=int, default=4, help="Number of entangling+rotation layers")
...
class SingleDiscoModel(nn.Module):
    """Wrapper exposing logits and score; uses ANO_VQC_Model as head for QNN."""
    def __init__(self, n_features: int = 11, n_classes: int = 2,
                 qnn_n_qubits: int = 11, qnn_n_outputs: int = 2,
                 qnn_n_local: int = 1, qnn_vqc_depth: int = 4,
                 qnn_device: str = "default.qubit") -> None:
        super().__init__()
        # Keep signature compatible; the QNN head ignores n_features/n_classes
        self.head = ANO_VQC_Model(
            n_qubits=int(qnn_n_qubits),
            n_outputs=int(qnn_n_outputs),
            n_local=int(qnn_n_local),
...
def make_hist_with_estimated_background(x: np.ndarray, labels: np.ndarray, weights: np.ndarray, regions: Dict[str, np.ndarray], bins: int, out: Path, title: str) -> None:
    """Stacked histogram of signal and true background in SR with estimated background overlays.

    - SR = region A mask (score pass & in m_hat window).
    - True SR background shape plotted as histogram.
    - Predicted background overlays:
        * B-shape scaled by C/D (uses region B shape multiplied by tf_C_over_D)
        * C-shape scaled by B/D (uses region C shape multiplied by tf_B_over_D)
    - Use weighted counts, normalise overlays to predicted SR normalization implied by ABCD.
    """
    A = regions["A"]
    B = regions["B"]
    C = regions["C"]
    ...
    D_bg = float(np.sum(w[D & b_mask]))
    eps = 1e-12
    tf_C_over_D = C_bg / (D_bg + eps)
    tf_B_over_D = B_bg / (D_bg + eps)

    # Hist in SR (A) for signal and true background
    x_range = (float(np.min(x)), float(np.max(x)))
    nb = bins
    plt.figure(figsize=(9, 6))
    # True background in A
    ...
    # Estimated background from B: scale B-shape by C/D
    bB_counts, _ = np.histogram(x[B & b_mask], bins=nb, range=x_range, weights=w[B & b_mask])
    est_from_B = tf_C_over_D * bB_counts
    # Estimated background from C: scale C-shape by B/D
    bC_counts, _ = np.histogram(x[C & b_mask], bins=nb, range=x_range, weights=w[C & b_mask])
    est_from_C = tf_B_over_D * bC_counts
    # Plot stacked true S+B and overlays
    centers = 0.5 * (b_edges[:-1] + b_edges[1:])
    width = (b_edges[1] - b_edges[0])
    plt.bar(centers, b_counts, width=0.9 * width, color="tab:blue", alpha=0.5, label="True background (SR)")
    plt.bar(centers, s_counts, bottom=b_counts, width=0.9 * width, color="tab:orange", alpha=0.5, label="Signal (SR)")
    # Clearer legend: both lines are estimates of the SR background using ABCD transfer factors
    plt.step(centers, est_from_B, where="mid", color="black", lw=2, label="Estimated background (from B×C/D)")
    plt.step(centers, est_from_C, where="mid", color="green", lw=2, label="Estimated background (from C×B/D)")
    plt.xlabel("Variable")
    plt.ylabel("Weighted events")
    plt.title(title + "\nABCD regions: A=pass&SR, B=pass&SB, C=fail&SR, D=fail&SB; TFs: B/D and C/D")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(out)
    plt.close()
...
def plot_logit_and_score_distributions(logits: np.ndarray, scores: np.ndarray, labels: np.ndarray, out_prefix: Path, title_prefix: str) -> None:
    ...
    for k in range(min(2, logits.shape[1])):
        ...
        plt.savefig(out_prefix.parent / f"{out_prefix.name}_logit{k}.png")
        plt.close()
    # Scores
    plt.figure(figsize=(8, 5))
    ...
    plt.legend(); plt.tight_layout()
    plt.savefig(out_prefix.parent / f"{out_prefix.name}_score.png")
    plt.close()
...
def main() -> None:
    ...
    # Enable multi-worker loading to overlap CPU I/O with compute
    num_workers = int(os.environ.get("QML_DATALOADER_WORKERS", "4"))
    train_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=num_workers, persistent_workers=(num_workers > 0))
    val_loader = DataLoader(valset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=num_workers, persistent_workers=(num_workers > 0))
    test_loader = DataLoader(testset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=num_workers, persistent_workers=(num_workers > 0))
    ...
    model = SingleDiscoModel(
        ...
        qnn_n_local=int(args.qnn_n_local),
    ...
    for epoch in range(start_epoch, int(args.epochs)):
        ...
        for split_name in ("train", "val"):
            ...
            out_path = dirs["plots_classification"] / f"roc_{split_name}_epoch_{epoch:03d}.png"
        ...
        for split_name in ("train", "val"):
            ...
            out_path = dirs["plots_classification"] / f"confusion_{split_name}_epoch_{epoch:03d}.png"
            plot_confusion(cm, out_path, f"Confusion ({split_name}) @ threshold=0.5 epoch {epoch}")
            out_path_norm = dirs["plots_classification"] / f"confusion_{split_name}_norm_epoch_{epoch:03d}.png"
            ...
            for wp, cut in ( ("10", split_metrics[split_name]["abcd"]["10"].get("cut", float("nan"))),
                ...
                out_path_wp = dirs["plots_classification"] / f"confusion_{split_name}_wp{wp}_epoch_{epoch:03d}.png"
                plot_confusion(cm_wp, out_path_wp, f"Confusion ({split_name}) @ εS={wp}% (threshold={cut:.4f}) epoch {epoch}")
                out_path_wp_norm = dirs["plots_classification"] / f"confusion_{split_name}_wp{wp}_norm_epoch_{epoch:03d}.png"
        ...
        for split_name in ("train", "val"):
            sm = split_metrics[split_name]
            out_prefix = dirs["plots_classification"] / f"dists_{split_name}_epoch_{epoch:03d}"
            plot_logit_and_score_distributions(sm["logits"], sm["scores"], sm["labels"], out_prefix, f"{split_name}")

        # 4) ABCD scatter plots (score vs m_hat)
        for split_name in ("train", "val"):
            sm = split_metrics[split_name]
            out_path = dirs["plots_abcd"] / f"scatter_score_vs_mhat_{split_name}_epoch_{epoch:03d}.png"
        ...
        # 5) Feature and m_hat histograms in SR with estimated background overlays (for 3 WPs)
        #    Produce overlays for m_hat at εS=10,30,50.
        for split_name in ("train", "val"):
            sm = split_metrics[split_name]
            for wp in ("10", "30", "50"):
                abcd_wp = sm["abcd"][wp]
                ...
                if regions is None:
                    continue
                # m_hat overlay per working point
                out_path = dirs["plots_abcd"] / f"hist_mhat_overlay_wp{wp}_{split_name}_epoch_{epoch:03d}.png"
                make_hist_with_estimated_background(sm["mhat"], sm["labels"], sm["weights"], regions, bins=60, out=out_path, title=f"m_hat overlays ({split_name}) @ εS={wp}% epoch {epoch}")
            # Plain 1D histograms for m_hat (signal vs background)
            plt.figure(figsize=(8,5))
            ...
            plt.tight_layout(); plt.savefig(dirs["plots_features"] / f"mhat_hist_{split_name}_epoch_{epoch:03d}.png"); plt.close()
        ...
        def plot_feature_hists(x: np.ndarray, y: np.ndarray, split: str) -> None:
            ...
            for i in range(nfeat):
                ...
                plt.tight_layout(); plt.savefig(dirs["plots_features"] / f"feature_{safe}_hist_{split}_epoch_{epoch:03d}.png"); plt.close()
    ...
    try:
        ...
        plt.tight_layout(); plt.savefig(dirs["plots_history"] / "auc_vs_epoch.png"); plt.close()
        ...
        plt.tight_layout(); plt.savefig(dirs["plots_history"] / "losses_vs_epoch.png"); plt.close()
        ...
        for metric in ("accuracy", "precision", "recall", "f1"):
            ...
            plt.tight_layout(); plt.savefig(dirs["plots_history"] / f"{metric}_vs_epoch.png"); plt.close()
        ...
        for metric in ("accuracy", "precision", "recall", "f1"):
            ...
            plt.tight_layout(); plt.savefig(dirs["plots_history"] / f"{metric}_wps_vs_epoch.png"); plt.close()
        ...
        for mkey, ylabel in (("b_eff", "background efficiency ε_B"), ("b_rej", "background rejection 1/ε_B"), ("sig_cont", "signal contamination")):
            ...
            plt.legend(ncol=2); plt.grid(True, alpha=0.3)
            plt.tight_layout(); plt.savefig(dirs["plots_history"] / f"{mkey}_vs_epoch.png"); plt.close()
        ...
        plt.legend(ncol=2); plt.grid(True, alpha=0.3)
        plt.tight_layout(); plt.savefig(dirs["plots_history"] / "jsd_vs_epoch.png"); plt.close()
        ...
        plt.legend(ncol=2); plt.grid(True, alpha=0.3)
        plt.tight_layout(); plt.savefig(dirs["plots_history"] / "inv_jsd_vs_epoch.png"); plt.close()
        ...
        plt.legend(ncol=2); plt.grid(True, alpha=0.3)
        plt.tight_layout(); plt.savefig(dirs["plots_history"] / "closure_vs_epoch.png"); plt.close()
