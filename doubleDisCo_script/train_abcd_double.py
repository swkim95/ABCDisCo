...
"""
Double-DisCo training script (Torch) with two DNNs and full evaluation.

What this implements (compared to the single-DisCo script):
- Two separate DNN classifiers trained jointly on the same 13 HLF inputs
  (f: x-axis, g: y-axis of ABCD). Each produces its own logits and score.
...
- 2D ABCD evaluation at εS = 10/30/50% by scanning (cut_f, cut_g) pairs and
  selecting the pair with closure closest to 1 (ties broken by higher 1/ε_B).
- JSDvsR computed using the two DNN outputs (no m_hat): threshold on f (x-axis)
  and treat g (y-axis) as the mass-like variable for background distribution
  comparison; we report b_rej and 1/JSD as in the ABCDisCo paper.
- Per-model (f and g) metrics and plots: ROC/AUC, confusion matrices (0.5 and
  the 10/30/50% working points), logits and score distributions, and history.
- Scatter of f vs g with WP lines, and histograms with estimated background
  overlays for both variables using ABCD regions.
- Complete per-epoch NPZ exports containing logits/scores for both models for
  reproducibility.

Physics/QML mapping (for HEP readers):
- Inputs: 13 HLF observables = [mass, pT, tau*_half, tau*, tau*_sq]; globally
  min–max scaled. No removal of mass or pT for Double-DisCo.
- DisCo decorrelation uses distance correlation (unbiased) between the two
  model scores restricted to QCD background, promoting independence between the
  two axes used in ABCD.
- ABCD uses (x=f score, y=g score). A=pass_f & pass_g; B=pass_f & fail_g;
  C=fail_f & pass_g; D=fail_f & fail_g. Background prediction in A is B×C/D.
...
Usage (HPC hint):
- Run `setup` in your shell first (loads the ABCDisCo conda env).
- Example:
  python doubleDisCo_script/train_abcd_double.py --smear 25 --gpunum 0 \
         --run_label smear25
"""
...
from networks import DNNclassifier, DNNclassifier_matching_HybridQNN
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Double-DisCo training (two DNNs on 13 features) with full diagnostics"
    )
...
def compute_auc(scores: np.ndarray, labels: np.ndarray, weights: np.ndarray) -> float:
    """Compute ROC AUC via weighted Mann–Whitney equivalence (no sklearn)."""
    s_mask = labels > 0.5
    b_mask = ~s_mask
...
def load_and_preprocess(smear_sigma: float, test: bool = False) -> Dict[str, Any]:
    ...
    stacked = np.hstack(
        (
            obs_scaled.astype(np.float32),  # 13 scaled observables
    ...
    # Datasets using ALL 13 features (columns up to -4)
...
def print_dataset_summary_split(name: str, x: np.ndarray, y: np.ndarray, out_dir: Path, num_rows_preview: int = 10) -> None:
    """Save a textual summary of the split: counts and feature ranges."""
    total = x.shape[0]
    ...
    # First rows for quick inspection
...
def plot_feature_distributions_once(train_x: np.ndarray, val_x: np.ndarray, test_x: np.ndarray,
    ...
    """Draw once the 1D histograms of signal and background for all 13 input features."""
    ensure_dir(out_dir)
...
class DoubleDiscoModel(nn.Module):
    """Two-head DNN: f (x-axis) and g (y-axis) using the same 13 inputs."""
    def __init__(self, n_features: int = 13, n_classes: int = 2) -> None:
        super().__init__()
        self.f_head = DNNclassifier_matching_HybridQNN(n_features, n_classes)
        self.g_head = DNNclassifier_matching_HybridQNN(n_features, n_classes)
        # self.f_head = DNNclassifier(n_features, n_classes)
        # self.g_head = DNNclassifier(n_features, n_classes)
...
def compute_losses_for_batch_double(model: nn.Module, batch: Tuple[torch.Tensor, ...], alpha: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Compute BCE losses for f and g, plus unbiased distance-correlation between f and g on background.
    ...
    # Background-only decorrelation penalty between f and g
...
def collect_epoch_arrays_double(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, np.ndarray]:
    """Run inference to collect logits/scores for f and g and other arrays for a full split."""
    model.eval()
...
def run_training_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device, alpha: float) -> Dict[str, float]:
    """One training epoch; returns aggregated losses and speed stats."""
    model.train()
...
##### ABCD utilities (Double-DisCo, two axes f and g) ########################################
...
def compute_abcd_yields(scores_f: np.ndarray, scores_g: np.ndarray, labels: np.ndarray, weights: np.ndarray, cut_f: float, cut_g: float) -> Dict[str, float]:
    """Compute weighted background yields in A/B/C/D for given (cut_f, cut_g)."""
    s_mask = labels > 0.5
    b_mask = ~s_mask
...
def generate_candidate_cut_pairs(scores_f: np.ndarray, scores_g: np.ndarray, labels: np.ndarray, weights: np.ndarray, target_eff: float, num_grid: int = 40) -> List[Tuple[float, float]]:
    """Generate (cut_f, cut_g) pairs such that total signal efficiency ≈ target_eff.

    Strategy mirrors evaluation.create_y1y2list but uses weighted quantiles and both scan orders.
    """
    ...
    # Scan eS1 from target to 1; set eS2 = target / eS1, define cuts conditionally
    ...
    # Symmetric scan: scan on g first
...
def compute_abcd_metrics_scan(scores_f: np.ndarray, scores_g: np.ndarray, labels: np.ndarray, weights: np.ndarray, target_eff: float) -> Dict[str, Any]:
    """Scan (cut_f, cut_g) pairs and return best by |closure-1| (tie-break higher 1/ε_B)."""
    ...
    best = None
    best_key = (float("inf"), -float("inf"))  # (abs(closure-1), -b_rej)
    ...
    for (cut_f, cut_g) in candidates:
        ...
        # Estimate background rejection from 2D (as 1/ε_B for passing region f&g)
        b_mask = labels < 0.5
        w = weights.astype(float)
...
def plot_scatter_f_vs_g(scores_f: np.ndarray, scores_g: np.ndarray, labels: np.ndarray, out: Path, title: str, max_points: int = 100000, cuts_by_wp: Optional[Dict[str, Tuple[float, float]]] = None) -> None:
    """Scatter of f vs g scores with WP cut lines."""
    n = len(scores_f)
    if n > max_points:
...
def make_hist_with_estimated_background(x: np.ndarray, labels: np.ndarray, weights: np.ndarray, regions: Dict[str, np.ndarray], bins: int, out: Path, title: str, x_label: Optional[str] = None, x_range: Optional[Tuple[float, float]] = None, log_scale: bool = False) -> None:
    """Stacked histogram of signal and true background in SR with estimated background overlay and ratio panel."""
    A = regions["A"]
    B = regions["B"]
    C = regions["C"]
    D = regions["D"]
...
def plot_logit_and_score_distributions(logits: np.ndarray, scores: np.ndarray, labels: np.ndarray, out_root_dir: Path, file_stub: str, title_prefix: str) -> None:
    """Plot distributions for raw logits (per class) and softmax score (class-1)."""
...
def main() -> None:
    ...
    val_loader = DataLoader(valset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
    test_loader = DataLoader(testset, batch_size=args.batch_size, shuffle=False, pin_memory=True)

    # Dataset summaries saved to history dir
    print_dataset_summary_split(
        "train", payload["train_features_np"], payload["train_labels_np"], dirs["history"]
    )
    print_dataset_summary_split(
        "val", payload["val_features_np"], payload["val_labels_np"], dirs["history"]
    )
    print_dataset_summary_split(
        "test", payload["test_features_np"], payload["test_labels_np"], dirs["history"]
    )

    # Model & optimizer
    model = DoubleDiscoModel(n_features=13, n_classes=2).to(device)
    ...
    for epoch in range(start_epoch, int(args.epochs)):
        ...
        # Compute validation losses using a pass over val loader (no grad)
        model.eval()
        ...
        # Compute per-model classification metrics and 2D ABCD/JSD for both splits
        ...
        for split_name, arrays in (
            ...
            # Working-point metrics for each model
            ...
            # ABCD diagnostics (2D, pair scan) for 10/30/50
        ...
        for split_name in ("train", "val"):
            ...
            for model_key in ("f", "g"):
                ...
                # WPs
                for wp in ("10", "30", "50"):
                    cut = sm[f"metrics_wp_{model_key}"][wp]
                    # Recompute confusion at this cut
                    scores = sm[f"scores_{model_key}"]
                    labels = sm["labels"]
                    weights = sm["weights"]
        ...
        # 4) ABCD scatter plots (f vs g) with WP lines
        for split_name in ("train", "val"):
            sm = split_metrics[split_name]
            split_dir = "train" if split_name == "train" else "valid"
        ...
        # 5) Histogram overlays (f and g separately) using 2D regions from best pair per WP
        for split_name in ("train", "val"):
            sm = split_metrics[split_name]
            split_dir = "train" if split_name == "train" else "valid"
            for wp in ("10", "30", "50"):
        ...
        # Include per-model background metrics and WP metrics
        for split_name in ("train", "val"):
            for model_key in ("f", "g"):
        ...
        for split_name in ("train", "val"):
            for wp in ("10", "30", "50"):
                ...
                # Per-region signal contamination and normalized
        ...
        # Optionally persist arrays for complete reproducibility
        ...
        # Write a concise line to legacy log CSV (key,value pairs)
        legacy_log.write(
    ...
    try:
        ...
        # Losses vs epoch (totals and components)
        ...
        # Accuracy/Precision/Recall/F1 vs epoch for train/val per model (0.5 threshold)
        ...
        # Background metrics vs epoch at three WPs per model (b_eff, b_rej, sig_cont)
        ...
        # JSD and 1/JSD vs epoch (f->g) and Closure vs epoch (2D ABCD)
        plt.figure(figsize=(8,5))
        for wp, style in [("10","--"),("30","-"),("50",":")]:
    ...
    # ABCD test diagnostics at 30% using 2D scan
