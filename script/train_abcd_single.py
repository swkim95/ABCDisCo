...
class SingleDiscoModel(nn.Module):
    """Thin wrapper that preserves the original networks.DNNclassifier head and exposes logits and score."""
    def __init__(self, n_features: int = 11, n_classes: int = 2) -> None:
        super().__init__()
        # self.head = DNNclassifier(n_features, n_classes)
        self.head = DNNclassifier_matching_HybridQNN(n_features, n_classes)
...
def compute_abcd_metrics_with_mhat(scores: np.ndarray, labels: np.ndarray, weights: np.ndarray, mhat: np.ndarray, target_eff: float, m_top: float = 172.5, delta_m: float = 35.0) -> Dict[str, Any]:
    ...
    D = (~pass_score) & (~in_sr)
    w = weights.astype(float)
    # Background-only sums (for ABCD prediction and transfer factors)
...
def main() -> None:
    ...
    # Model & optimizer
    model = SingleDiscoModel(n_features=11, n_classes=2).to(device)
